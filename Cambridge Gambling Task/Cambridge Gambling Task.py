...
nBlocks = 8 # 8 blocks (4 for ascending, 4 for descending condition)
nTrials = 9 # 9 trials per block with different box ratios
...
Trials = [Trial1, Trial2, Trial3, Trial4, Trial5, Trial6, Trial7, Trial8, Trial9]
...
blockEnd = visual.TextStim(
            win, 
            units="pix",
            height=20,
            text="Final score for last game: %s\n\nPlease take this opportunity to have a short rest, and press ENTER when you are ready to continue."%(currentBank))            
...
for iBlock in range (0,nBlocks):
    ...
    blockEnd.text = ("Final score for last game: %s\n\nPlease take this opportunity to have a short rest, and press ENTER when you are ready to continue."%(currentBank))
    blockEnd.draw()
    win.flip()
    thisKey = event.waitKeys(keyList = expKeys)
    if thisKey == ['escape']:
