...
screenRes = [1920,1080] #define BEAM lab monitor size ([1024,768] in room 1, [1920,1080] in room 2, [1366, 768] at MM)
...
nBlocks = 5             # how many blocks will there be?
nTrialsPerBlock = 88    # how many trials between breaks?
blockLengths = [88,88,88,88,88]     # how many trials in each block? The length of this list will be the number of blocks.
...
flankerDur = 0.10      # time flanker arrows are onscreen before target (in seconds)
...
baseBlankDur = blankDur # base duration for blank screen whilst waiting for a response
...
bottomPrompts = ["Press any key to continue.         ",
    "No feedback will be provided. Let the experimenter know when you're ready to begin."]

endtopPrompts = ["This is the end of the experiment. Your average reaction time in this block was:\n                    %s ms. \n Your average accuracy was:\n                    %s %%."%(blockRTAve, blockAccAve)]
...
#    totalFrames=fixDurF+FFDf+flankerDurF+targetDurF
    frameN = 0
...
#    allKeys = event.getKeys(timeStamped=trialClock)
    ...
    for frameN in range(blankDurF):
        ...
        if checkResp == 1:
            ...
            if allKeys == []:
                ...
                break
    if breakTrial == True:
        blankDur = respBlankDur #NEEDS FIXING! There is a 500 ms blank after every response within 1500 ms of target onset -> another lvl of RT: during target display, within first 1000 ms, and base
        blankDurF = int(round(refreshRate*blankDur))
        for frameN in range(blankDurF):
            win.flip()
    ...
    if respRecorded == True:
        ...
        blockRTAve = int(round(np.median(blockRT)))
...
while iEnd < len(endtopPrompts):
    endtopPrompts = ["This is the end of the task - thank you! Your average reaction time in this block was:\n                    %s ms. \n Your average accuracy was:\n                    %s %%."%(blockRTAve, blockAccAve)] # update end message
